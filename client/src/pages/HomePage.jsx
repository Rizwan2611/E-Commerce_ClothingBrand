    ...
    <div className="pt-24 bg-zinc-50">
      ...
      <section className="relative min-h-[95vh] flex items-center justify-center overflow-hidden px-4 sm:px-6 lg:px-12 py-20">
        ...
        <div className="relative z-10 max-w-full w-full flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          ...
          <div className="flex-1 three-d-card glass-premium p-10 md:p-20 rounded-[4rem] border-2 border-white/50 depth-lg animate-slide-up text-center lg:text-left">
            <div className="inline-flex items-center gap-3 px-6 py-2.5 bg-green-400/10 border border-green-400/20 rounded-full text-green-500 font-bold text-[10px] uppercase tracking-[0.5em] mb-10">
              <Sparkles size={14} />
              <span>Spring Drop // 2026</span>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-10">
              ...
              <Link to="/location" className="text-black font-black font-rock-salt text-xs uppercase tracking-[0.3em] hover:text-green-500 transition-colors border-b-2 border-black pb-1">
                Our Studio
              </Link>
            </div>
          </div>
          ...
          <div className="flex-1 w-full relative perspective-1000 flex justify-center items-center h-[450px] md:h-[600px] mt-20 lg:mt-0">
            ...
            <div className="three-d-card w-[180px] md:w-[280px] aspect-[3/4] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden depth-md border-2 md:border-4 border-white shadow-xl absolute z-30 right-4 md:right-0 top-0 group rotate-[12deg] hover:rotate-0 -translate-x-5 md:-translate-x-10 translate-y-10 hover:translate-y-5">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-green-400/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-4 md:p-6">
                <span className="text-black font-rock-salt text-lg md:text-xl font-black uppercase tracking-tighter">Premium Denim</span>
              </div>
            </div>

            {/* Background Accent Element */}
            <div className="absolute -z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-green-400/20 rounded-full blur-[80px] md:blur-[120px] animate-pulse-glow" />
          </div>
        </div>
      </section>
      ...
      <section className="py-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="flex flex-col items-center mb-20 text-center">
          <span className="text-xs font-[1000] text-green-500 uppercase tracking-[0.5em] mb-4">Curated Selections</span>
          ...
          {categories.map((cat) => (
            <Link key={cat.name} to={cat.path} className="three-d-card group bg-white rounded-[2rem] p-4 border border-zinc-100 depth-sm hover:depth-lg flex flex-col">
              <div className="w-full aspect-[4/5] rounded-[1.5rem] overflow-hidden mb-6 relative shadow-inner">
                <div className="absolute inset-0 bg-black/5 group-hover:bg-green-400/10 transition-colors duration-500 z-10" />
                <img src={cat.image} alt={cat.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
              </div>
              <div className="flex justify-center items-center pb-4">
                <span className="text-xl font-[1000] uppercase tracking-tighter text-black group-hover:text-green-500 transition-all duration-300">
                  {cat.name}
                </span>
              </div>
            </Link>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative overflow-hidden">
        ...
        <div className="flex items-end justify-between mb-12 relative z-10">
          ...
          <Link to="/shop" className="group flex items-center gap-4 text-black hover:text-green-500 text-[10px] font-black transition-all uppercase tracking-[0.2em]">
            <span className="border-b-2 border-black group-hover:border-green-500 pb-1">Explore More</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              ...
              <div key={title} className="three-d-card group flex flex-col items-center text-center p-10 rounded-[2.5rem] bg-white border border-zinc-100 depth-md hover:depth-lg">
                <div className="w-20 h-20 bg-zinc-50 border-2 border-zinc-100 group-hover:border-green-400 group-hover:bg-green-400/10 rounded-[1.5rem] flex items-center justify-center mb-8 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500 shadow-inner">
                  <Icon size={32} className="text-black group-hover:text-green-500 transition-colors" />
                </div>
                <h3 className="text-2xl font-black text-black mb-4 tracking-tighter font-rock-salt group-hover:text-green-500 transition-colors uppercase">{title}</h3>
        ...
        <div className="max-w-full mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div>
            ...
            <ul className="space-y-4">
              <li><Link to="/shop" className="text-zinc-600 hover:text-green-500 transition-colors font-medium">Shop Collection</Link></li>
              <li><Link to="/location" className="text-zinc-600 hover:text-green-500 transition-colors font-medium">Find Us</Link></li>
              <li><Link to="/orders" className="text-zinc-600 hover:text-green-500 transition-colors font-medium">My Orders</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex-1 py-3 border border-black rounded-xl text-center text-xs font-black hover:bg-green-400 hover:text-black hover:border-green-400 transition-all uppercase"
              >
                Instagram
              </a>
              <a
